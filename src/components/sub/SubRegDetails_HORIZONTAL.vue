  ...
  <v-container v-if="macAddress" grid-list-lg>
    ...
    <v-layout class="ma-1" row wrap align-center justify-center>
      ...
      <v-layout class="ma-4" column align-center justify-space-around>
        ...
        <v-flex shrink>
          <h2 v-if="$vuetify.breakpoint.mdAndUp" class="headline font-weight-light">Lets get you started with your connected droplet</h2>
          <h2 v-if="$vuetify.breakpoint.smAndDown" class="font font-weight-thin text-center">Lets get you started with your connected droplet</h2>
        </v-flex>
      </v-layout>
    </v-layout>
    </v-fade-transition>
...
export default {
  ...
  mounted () {
    ...
    const sanitizedMacAddress = this.sanitizeMacAddress(this.query)

    if (!sanitizedMacAddress || !sanitizedMacAddress.match(this.macAddressRegEx)) {
      this.showAlert('Invalid Connected Droplet Address.', '/error')
    } else {
